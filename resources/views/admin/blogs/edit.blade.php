            ...
            <form method="POST" action="{{ route('admin.blogs.update', [$blog->id]) }}" enctype="multipart/form-data">
                ...
                <div class="row justify-content-center">
                    <div class="form-group col-4">
                        ...
                        <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id"
                            ...
                            @foreach ($users as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('user_id') ? old('user_id') : $blog->user->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('user'))
                    ...
                    <div class="form-group col-4">
                        ...
                        <input class="form-control {{ $errors->has('media_url') ? 'is-invalid' : '' }}" type="text"
                            name="media_url" id="media_url" value="{{ old('media_url', $blog->media_url) }}">
                    ...
                    <div class="form-group col-4">
                        ...
                        <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type"
                            ...
                            @foreach (App\Models\Blog::TYPE_SELECT as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('type', $blog->type) === (string) $key ? 'selected' : '' }}>{{ $label }}
                                </option>
                            @endforeach
                        </select>
                        @if ($errors->has('type'))
                ...
                <div class="row justify-content-center">
                    ...
                    <div class="form-group col-4">
                        ...
                        <textarea class="form-control ckeditor {{ $errors->has('content') ? 'is-invalid' : '' }}" name="content"
                            id="content">{!! old('content', $blog->content) !!}</textarea>
                    ...
                    <div class="form-group col-4">
                        ...
                        <textarea class="form-control {{ $errors->has('short_description') ? 'is-invalid' : '' }}" name="short_description"
                            id="short_description" required>{{ old('short_description', $blog->short_description) }}</textarea>
                ...
                <div class="row justify-content-center">
                    ...
                    <div class="form-group col-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('blog_comments') ? 'is-invalid' : '' }}"
                            name="blog_comments[]" id="blog_comments" multiple>
                            @foreach ($blog_comments as $id => $blog_comment)
                                <option value="{{ $id }}"
                                    {{ in_array($id, old('blog_comments', [])) || $blog->blog_comments->contains($id) ? 'selected' : '' }}>
                                    {{ $blog_comment }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]"
                            ...
                            @foreach ($tags as $id => $tag)
                                <option value="{{ $id }}"
                                    {{ in_array($id, old('tags', [])) || $blog->tags->contains($id) ? 'selected' : '' }}>
                                    {{ $tag }}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('tags'))
