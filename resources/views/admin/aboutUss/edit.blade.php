            ...
            <form method="POST" action="{{ route('admin.about-uss.update', [$aboutUss->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" type="text"
                        name="phone_number" id="phone_number" value="{{ old('phone_number', $aboutUss->phone_number) }}">
                ...
                <div class="form-group">
                    <label class="required"
                        for="normal_shipment_cost">{{ trans('cruds.aboutUs.fields.normal_shipment_cost') }}</label>
                    <input class="form-control {{ $errors->has('normal_shipment_cost') ? 'is-invalid' : '' }}"
                        type="number" name="normal_shipment_cost" id="normal_shipment_cost"
                        value="{{ old('normal_shipment_cost', $aboutUss->normal_shipment_cost) }}" step="0.01" required>
                ...
                <div class="form-group">
                    <label class="required"
                        for="fast_shipment_cost">{{ trans('cruds.aboutUs.fields.fast_shipment_cost') }}</label>
                    <input class="form-control {{ $errors->has('fast_shipment_cost') ? 'is-invalid' : '' }}" type="number"
                        name="fast_shipment_cost" id="fast_shipment_cost"
                        value="{{ old('fast_shipment_cost', $aboutUss->fast_shipment_cost) }}" step="0.01" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                        name="name" id="name" value="{{ old('name', $aboutUss->name) }}" required>
                    @if ($errors->has('name'))
                        <div class="invalid-feedback">
                            {{ $errors->first('name') }}
                        </div>
