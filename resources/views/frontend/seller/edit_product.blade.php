...
<div class=" card-body bg-gray-100 flex-1 p-6 md:mt-16" style="direction: rtl;">
    ...
    <form method="POST" action="{{ route("seller.products.update",$product->id) }}" enctype="multipart/form-data">
            ...
            <div class="addpro">
                <div class=" form-group mt-6">
                    ...
                    <select class="form-control select2 mt-3 {{ $errors->has('product_category') ? 'is-invalid' : '' }}"
                        ...
                        <option value="{{ $id }}" {{ $product->product_category_id==$id ? 'selected' : '' }}>{{ $entry }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="addpro">
                <div class=" form-group mt-6">
                    ...
                    <input class="form-control mt-3{{ $errors->has('current_stock') ? 'is-invalid' : '' }}"
                        type="number" name="current_stock" id="current_stock" value="{{ old('current_stock', $product->current_stock) }}"
                        step="1" required>
            ...
            <div class="addpro">
                <div class=" form-group mt-6">
                    ...
                    <textarea class="form-control ckeditor{{ $errors->has('information') ? 'is-invalid' : '' }}"
                        name="information" id="information">{!! $product->information !!}</textarea>
                ...
                <div class="form-group mt-6">
                    ...
                    <select class="form-control {{ $errors->has('shipping_method') ? 'is-invalid' : '' }}" name="shipping_method" id="shipping_method" required>
                        ...
                        @foreach(App\Models\Product::SHIPPING_METHOD_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ $product->shipping_method === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-1 mt-10">
            <button type="submit" class=" btn-bs-dark mr-6 lg:mr-0 lg:mb-6"> تعديل المنتج</button>
        </div>
        <!-- end content -->
    </form>
